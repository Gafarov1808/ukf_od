...
from scipy.linalg import cholesky, ldl
    ...
    def cholupdate(self, R: np.ndarray, u: np.ndarray, v: float) -> np.ndarray:
        ...
        for k in range(n):
            ...
            if v > 0:
                ...
                if r_sq_new <= 0.0:
                    L, d, _ = ldl(R.T @ R - np.outer(u,u))
                    d_reg = np.maximum(d, 1e-8)
                    R_new = L @ np.diag(np.sqrt(d_reg))
                    R_new = np.triu(R_new)
                    #print('LDL-разложение с регуляризацией')
                    return R_new
                    #raise ValueError('cholupdate. Пропуск оценки.')
                else:
                    r_new = np.sqrt(r_sq_new)
                    if abs(r_kk) > 1e-16:
                        c = r_new / r_kk
